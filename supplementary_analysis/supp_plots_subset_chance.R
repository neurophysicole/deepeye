...
# --------
# timeline data
m.data    <- c(.692, .687, .712, .720, .675, .656) #means
...
plot_labs   <- labs(x = 'Supplemental Analysis', y = '') #customize axis and legend labels
...
hz_line_lab <- geom_text(aes(0, chance, label = '', hjust = 'left', vjust = -1, fontface = 'plain', family = 'Helvetica')) #Chance line label
...
themez     <- theme_apa(legend.pos = 'right', legend.font.size = '12')

zplot       <- plot + lims + plot_labs + bars + error_bars + colors + placement + hz_line + hz_line_lab + bar_labs + themez + theme(axis.text.y = element_blank(), axis.ticks.y = element_blank())
zplot
