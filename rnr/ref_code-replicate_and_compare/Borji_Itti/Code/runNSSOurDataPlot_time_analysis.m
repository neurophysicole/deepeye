...
%       load(['aliYarbus/fix/' subjList(i).name]);   
    for j=1:length(stimuliFilename)
        % for saccade
        x=round(eyeSaccade{j}(:,1)); numSacc = fix(length(x)/6); x = x((timeIdx-1)*numSacc+1:timeIdx*numSacc); 
        y=round(eyeSaccade{j}(:,2)); numSacc = fix(length(y)/6); y = y((timeIdx-1)*numSacc+1:timeIdx*numSacc); 
        
%         % for fixation
%         x=round(eyeSaccade{j}(:,1)); x = x((timeIdx-1)*5000+1:timeIdx*5000);
%         y=round(eyeSaccade{j}(:,2)); y = y((timeIdx-1)*5000+1:timeIdx*5000);
...
save(['./timeAnalysisNss/myNSSmoreAll_33_100_OurDataSacc-timeIdx-' num2str(timeIdx) '.mat'],'-v7.3','myNSS','fixMap','blurMap')
% save(['./timeAnalysisNss/myNSSmoreAll_33_100_OurDataFix-timeIdx-' num2str(timeIdx) '.mat'],'-v7.3','myNSS','fixMap','blurMap')
