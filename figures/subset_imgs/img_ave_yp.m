...
output_file_pattern = 'ave_yp.png';
...
for i = 1:n_subs
    ...
    for j = 1:n_trials
        ...
        % -- changed for ave
        datax = datax + (trial_data(:,1)*0);
        datay = datay + trial_data(:,2);
        markerdata = markerdata + (trial_data(:,3));
...
markerdata  = markerdata/ num_files;
markerSize  = ((markerdata/10)+1);

colormap(gray);
colormap(1-(gray/255)*255);
